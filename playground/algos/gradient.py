...
class PolicyGradientAgent(Agent):
    ...
    def policy_gradient(self):
        with tf.variable_scope("policy"):
            ...
            observation = tf.placeholder("float", [None, 4])
            actions = tf.placeholder("float", [None, 2])
            advantages = tf.placeholder("float", [None, 1])
            linear = tf.matmul(observation, parameters)
            probabilities = tf.nn.softmax(linear)
            good_probabilities = tf.reduce_sum(tf.mul(probabilities, actions), reduction_indices=[1])
            eligibility = tf.log(good_probabilities) * advantages
            loss = -tf.reduce_sum(eligibility)
            optimizer = tf.train.AdamOptimizer(0.01).minimize(loss)
            return probabilities, observation, actions, advantages, optimizer
    ...
    def value_gradient(self):
        with tf.variable_scope("value"):
            ...
            h1 = tf.nn.relu(tf.matmul(observation, w1) + b1)
